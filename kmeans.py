...
# ---
# VAE MODEL DEFINITION REMOVED
# ---

# ---
# ANALYSIS PIPELINE (MODIFIED: VAE REMOVED)
# ---

def run_analysis_pipeline(train_df, test_df, feature_cols, persona_map, visualization_method='tsne', title_prefix=""):
    """
    Runs the full pipeline, NOW WITH K-MEANS ONLY:
    1. StandardScaler
    2. K-Means (on scaled features)
    3. Visualization (on scaled features)
    """
    ...
    if feature_cols:
        ...
        # --- FIX: Map patient IDs to persona labels for metrics ---
        ...
        # --- FIX: Map patient IDs to persona labels for metrics ---
        train_labels = train_df['patient'].map(persona_map).values
        test_labels = test_df['patient'].map(persona_map).values
        # --- END FIX ---
    ...
    # --- VAE FEATURE EXTRACTION REMOVED ---

    # *** CRITICAL: All subsequent steps now use the SCALED features ***
    train_features = train_features_scaled
    test_features = test_features_scaled

    # --- 2. K-Means Clustering (on SCALED features) ---
    n_clusters = 25
    ...
    print("\n--- Clustering Evaluation Metrics (Train, on Scaled Features) ---")
    ...
    try:
        ...
        print("\n--- Clustering Evaluation Metrics (Test, on Scaled Features) ---")
    ...
    print(f"Applying {visualization_method.upper()} for visualization (on Scaled features)...")

    # --- 3. Dimensionality Reduction (on SCALED features) ---
    if train_features.shape[1] < 2:
        print("Warning: Fewer than 2 features. Visualizing raw features.")
    ...
    plot_train_df = pd.DataFrame({
        ...
        'patient': train_patient_ids # Use original patient IDs for plotting
    })
    plot_test_df = pd.DataFrame({
        'vis_comp_0': test_features_2d[:, 0],
        'vis_comp_1': test_features_2d[:, 1],
        'cluster': test_cluster_labels,
        'patient': test_patient_ids # Use original patient IDs for plotting
    })
    ...
    plot_ax.set_title(f"Visualization: {visualization_method.upper()} (on Scaled Raw Features)", fontsize=14)
    ...
    print("\nCalculating Mahalanobis distances (on Scaled Raw features)...")
    # Calculate Mahalanobis on SCALED features
    ...
    plot_train_df['distance'] = calculate_mahalanobis(train_features, cluster_means, cluster_inv_covs, plot_train_df['cluster'].values) # Scaled features
    plot_test_df['distance'] = calculate_mahalanobis(test_features, cluster_means, cluster_inv_covs, plot_test_df['cluster'].values)   # Scaled features
...
# ---
# FEATURIZATION (FIX 4 - No session_id)
# ---
